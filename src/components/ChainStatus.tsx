    ...
    <div className="glass-panel rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-base text-on-surface-variant">hub</span>
          <span className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Chain Link</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className={`w-2 h-2 rounded-full ${reachable ? "bg-green-500 pulse-green" : error ? "bg-rose-500" : "bg-zinc-600"}`} />
          <span className={`text-[10px] font-bold uppercase tracking-wider ${reachable ? "text-green-400" : error ? "text-rose-400" : "text-zinc-500"}`}>
      ...
      <div className="flex items-center justify-between py-2 border-t border-outline-variant/50">
        ...
        <span className="font-data-mono text-xs text-on-surface">{lastSync ? timeAgo(lastSync) : "—"}</span>
      </div>

      <div className="flex items-center justify-between py-2 border-t border-outline-variant/50">
        <span className="text-xs text-on-surface-variant">Contract</span>
        <a href={explorerContractUrl()} target="_blank" rel="noreferrer" className="font-data-mono text-[10px] text-primary hover:underline">
      ...
      {error && <p className="text-[10px] text-rose-400 mt-2 leading-relaxed">{error}</p>}
    </div>
  );
}

// Memoized — only re-renders when its props actually change
export const ChainStatus = memo(ChainStatusBase);
